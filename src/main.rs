...
fn main() {
    ...
    let data1: Vec<Data> = (0..DATA_SIZE)
        .map(|_| Data::from_usize(rng.gen::<usize>()))
        .collect();
    let data2: Vec<Data> = (0..DATA_SIZE)
        .map(|_| Data::from_usize(rng.gen::<usize>()))
        .collect();

    // Encrypt the data
    let start = Instant::now();
    let encrypted_data1: Vec<EncryptedData> =
        data1.iter().map(|data| data.encrypt(&context)).collect();
    let elapsed = start.elapsed();
    println!("Time needed to encrypt {} data: {:?}", DATA_SIZE, elapsed);
    println!(
        "Time needed to encrypt 1 data: {:?}",
        elapsed / DATA_SIZE as u32
    );
    let encrypted_data2: Vec<EncryptedData> = data2
        .par_iter()
        .map(|data| data.encrypt(&context))
        .collect();

    let start = Instant::now();
    let _decrypted_data: Vec<Data> = encrypted_data1
        .iter()
        .map(|data| data.decrypt(&context))
        .collect();
    let elapsed = start.elapsed();
    ...
    // Decrypt the result
    let start = Instant::now();
    let decrypted_data_add: Vec<Data> = encrypted_data3
        .iter()
        .map(|data| data.decrypt(&context))
        .collect();
    let elapsed = start.elapsed();
    ...
    let data3: Vec<Data> = data1
        .par_iter()
        .zip(data2.par_iter())
        .map(|(data1, data2)| data1.add_as_uint(&data2))
        .collect();
